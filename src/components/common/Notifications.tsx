...
function Notifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await fetch(`https://54.169.199.32:5000/api/user/get-user-notification`, {
        method: 'GET',
        headers: new Headers({
  ...
  const deleteNotifications = async (id: string) => {
    ...
    const token = session?.user?.accessToken;
    try {
      const response = await fetch(`https://54.169.199.32:5000/api/user/get-user-notification/${id}`, {
        method: 'DELETE',
        headers: new Headers({
          Authorization: `Bearer ${token}`,
        }),
