'use client';
  ...
  setLikeToggle(!likeToggle);
    ...
    const apiUrl = `https://54.169.199.32:5000/api/feed/posts?page=${page}`;
    ...
    const fetchComments = async (postId: string) => {
        try {
            if (!session) return;
            const token  = session.user?.accessToken;
          const response = await axios.get(`https://54.169.199.32:5000/api/comment/${postId}`, {}, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
    ...
    const handleCreateComment = async (postId: string, commentText: string) => {
        try {
            ...
            const response = await axios.post(
                `https://54.169.199.32:5000/api/comment/${postId}`,
                {
                    commentText
                },
                 {
    ...
    const sharePost = async (postId: string) => {
      ...
      try {
        const token = session.user?.accessToken;
        const response = await axios.get(`https://54.169.199.32:5000/api/feed/share/${postId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
